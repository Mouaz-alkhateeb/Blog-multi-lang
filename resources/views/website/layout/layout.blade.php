    ...
    <div class="container-fluid">
        ...
        <div class="row align-items-center py-2 px-lg-5">
            ...
            <div class="col-lg-8 text-center text-lg-right">
                <img class="img-fluid" src="{{ asset($settings->logo) }}" alt="" style="height: 70px">
            </div>
        </div>
    </div>
    <!-- Topbar End -->
                ...
                <div class="navbar-nav mr-auto py-0">
                    ...
                    @foreach ($categories as $category)
                        <div class="nav-item dropdown">
                            <a  @if (count($category->child) == 0) href="{{Route('category',$category->id)}}" @else href='#' @endif class="nav-link  @if (count($category->child) > 0) dropdown-toggle  @endif"
                            @if(count($category->child) > 0)  data-toggle="dropdown" @endif
                                >{{ $category->title }}</a>

                            @if (count($category->child) > 0)
                                <div class="dropdown-menu rounded-0 m-0">
                                    @foreach ($category->child as $ch)
                                        <a href="{{Route('category',$ch->id)}}" class="dropdown-item">{{ $ch->title }}</a>
                                    @endforeach
